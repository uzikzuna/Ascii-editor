...
import { 
  Camera, CameraOff, ZoomIn, ZoomOut
} from 'lucide-react';
...
export const WebcamConverter: React.FC = () => {
  ...
  const startCamera = async () => {
    try {
      ...
      setIsCapturing(true);
      showToast('Camera channel online!', 'success');
    } catch (err) {
      console.error(err);
      showToast('Could not access camera feed. Verify permissions.', 'error');
    }
  };
        ...
        {showControls && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
              {/* Webcam state indicator */}
              <div className="glass border border-theme-border/40 p-3 rounded-lg flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-theme-text font-sans">CAMERA STATUS</span>
                ...
                <p className="text-[10px] text-theme-muted font-mono leading-relaxed">
                  {isCapturing ? 'STREAMING ACTIVE [640x480 @ 30 FPS]' : 'LENS LINK DISCONNECTED'}
                </p>
                <div className="mt-2">
                  {isCapturing ? (
                    <Button variant="danger" size="sm" onClick={stopCamera} className="w-full">
                      <CameraOff className="w-4 h-4" />
                      <span>TERMINATE FEED</span>
                    </Button>
                  ) : (
                    <Button variant="primary" size="sm" onClick={startCamera} className="w-full">
              ...
              <div className="flex flex-col gap-3">
                <Slider label="COLUMNS (WIDTH)" min={20} max={160} step={2} value={options.width} onChange={(val) => setOptions(prev => ({ ...prev, width: val }))} />
                <Slider label="ROWS (HEIGHT)" min={10} max={100} step={2} value={options.height} onChange={(val) => setOptions(prev => ({ ...prev, height: val }))} />
              </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-black/40 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-auto flex items-center justify-center p-6 relative">
          {isCapturing ? (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-theme-accent/15 border border-theme-accent/30 shadow-neon flex items-center justify-center text-theme-accent animate-pulse">
                <Camera className="w-7 h-7" />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-theme-text">Start Webcam Feed</h3>
                <p className="text-xs text-theme-muted mt-2">
                  Grant permission to camera capture. Stream converts in real time to custom grid models.
                </p>
              </div>
            </div>
          )}
